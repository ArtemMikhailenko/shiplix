...
export default function ContactPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    // Client-side mailto fallback (no backend)
          ...
          <div className="lg:col-span-3">
            <form onSubmit={handleSubmit} className="fade-up space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <label htmlFor="name" className="block text-xs font-mono font-medium uppercase tracking-widest text-text-tertiary mb-2">
                ...
                <div>
                  <label htmlFor="email" className="block text-xs font-mono font-medium uppercase tracking-widest text-text-tertiary mb-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {BUDGET_KEYS.map((key) => (
                    <button
                      key={key}
                      type="button"
                      onClick={() => setFormData({ ...formData, budget: dict.contactPage.formBudgetOptions[key] })}
                      className={`text-sm font-medium px-4 py-2 rounded-pill transition-all duration-200 cursor-pointer ${
                        formData.budget === dict.contactPage.formBudgetOptions[key]
              ...
              <div>
                <label htmlFor="message" className="block text-xs font-mono font-medium uppercase tracking-widest text-text-tertiary mb-2">
              ...
              {status === "success" && (
                <p className="text-green text-sm">{dict.contactPage.formSuccess}</p>
              )}
            </form>
          </div>
